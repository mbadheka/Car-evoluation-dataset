RF <- function() {
  ...
  svm_Linear <- train(Condition ~., data = TrainSet, method = "svmLinear",
                      trControl=trctrl,
                      preProcess = c("center", "scale"),
                      tuneLength = 10)
  svm_Linear
  
  test_pred <- predict(svm_Linear, newdata = ValidationSet)
  confusionMatrix(test_pred, ValidationSet$Condition )
  
  grid <- expand.grid(C = c(0,0.01, 0.05, 0.1, 0.25, 0.5, 0.75, 1, 1.25, 1.5, 1.75, 2,5))
  set.seed(3233)
  svm_Linear_Grid <- train(Condition ~., data = TrainSet, method = "svmLinear",
                           trControl=trctrl,
                           preProcess = c("center", "scale"),
                           tuneGrid = grid,
                           tuneLength = 10)
  ...
  test_pred_grid_1 <- predict(svm_Linear_Grid, newdata = TestSet)
